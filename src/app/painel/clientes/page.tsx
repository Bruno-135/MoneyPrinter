      ...
      {clientes.length === 0 ? (
        ...
        <>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl border border-black/10 px-4 py-3.5 dark:border-white/10">
              <span className="block text-2xl font-semibold tabular-nums">{clientes.length}</span>
              <span className="text-xs opacity-55">clientes</span>
            </div>
            <div className="rounded-xl border border-emerald-600/30 bg-emerald-500/[0.06] px-4 py-3.5">
              ...
              <span className="text-xs opacity-55">por mês</span>
            </div>
            <div className="rounded-xl border border-black/10 px-4 py-3.5 dark:border-white/10">
              {totais.map(([moeda, t]) => (
              ...
              <span className="text-xs opacity-55">pagamentos únicos</span>
            </div>
            <div className="rounded-xl border border-black/10 px-4 py-3.5 dark:border-white/10">
              <span className="block text-2xl font-semibold tabular-nums">
                {clientes.reduce((s, c) => s + c.ativos.length, 0)}
              </span>
              <span className="text-xs opacity-55">serviços activos</span>
            </div>
          </section>
